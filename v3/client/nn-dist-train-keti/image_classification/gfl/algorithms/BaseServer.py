...
# <keti_code> --------------------------------
is_redis = True # Hard coding, 입력 인자로 수정 필요합니다.
...
class BaseServer:
    def __init__(
        ...
        print(f' type(optimizer) = {optimizer} ')
        if is_redis:
            # Redis에 server_optimizer 저장 (client로 전송할 수 있음)
            rediscon.set_ordered_dict(
                key = 'fl:server_optimizer',
                ordered_dict_model = optimizer
            )
    ...
    # 실행하려면 main.py 수정, 동기화 및 추가 작업이 필요합니다.
    #-------------------------------------------------------------------
    def _clients_training_with_redis(self, sampled_clients):
        ...
        for client_idx in sampled_clients:
            ...
            ## Download global
            self.client.download_global(None, None)
