...
class BaseServer:
    def __init__(
        ...
        # Server가 클라이언트에게 model과 optimizer의 제공하는 state_dict()정보를 제공합니다.
        # 이를 Redis에 기록합니다.    
        # <keti_code> --------------------------------
        print(f'[d] type(optimizer) = {optimizer} ')
        if is_redis:
            # model.state_dict()
            rediscon.set_ordered_dict(
                key = 'fl:server_weights',
                ordered_dict_model = model.state_dict()
            )
            
            # optimizer.state_dict()
            rediscon.set_ordered_dict(
                key = 'fl:server_optimizer',
                ordered_dict_model = optimizer.state_dict()
            )
        # <keti_code> --------------------------------
    ...
    def run(self):
        ...
        for round_idx in range(self.n_rounds):
            ...
            print(f'[d] Round index = {round_idx}')

            start_time = time.time()

            # Make local sets to distributed to clients
            sampled_clients = self._client_sampling(round_idx)
            print(f'[d] Index list of sampled_clients = {sampled_clients}')
            self.server_results["client_history"].append(sampled_clients)
            
            # Client training stage to upload weights & stats
            updated_local_weights, client_sizes, round_results = self._clients_training(sampled_clients) # round results: 모든 select된 local model들의 local train dataset 기준 train acc만!!(light version에서는 시간 줄이기 위함!!)
    ...
    def _clients_training(self, sampled_clients):
        if is_redis:
            return self._clients_training_with_redis(sampled_clients)
        else:
            return self._clients_training_for_simulation(sampled_clients)

    def _clients_training_for_simulation(self, sampled_clients):
        """Conduct local training and get trained local models' weights"""
        
        updated_local_weights, client_sizes = [], []
        round_results = {}

        # 내부 메모리에서 글로벌 모델 읽어오기
        ...
        for client_idx in sampled_clients:
            ...
            # Download global            
            # server로부터 해당 round의 initial model, momentum, weightdecay, lr 따옴!
            ...
            # Upload locals
            updated_local_weights.append(self.client.upload_local(client_idx))
                
            # Update results
            #select된 모든 client의 train acc(local data 기준)를 dictionary에 저장(light version)
            round_results = self._results_updater(round_results, local_results)
            
            #select된 모든 client의 train data의 size들을 저장
            client_sizes.append(local_size)
 
            # Reset local model
            self.client.reset()

        #select된 client에 해당하는 local model, train data수, local model의 train acc(local data 기준)(light version)
        return updated_local_weights, client_sizes, round_results 

    
    #-------------------------------------------------------------------
    # [주의] 아래 메서드는 클라이언트 작업을 물리적으로 분리하기 위해 
    # client별로 처리 결과를 Redis 서버에 저장합니다.
    # 다만 BaseServer.py와 BaseClientTrainer.py를 물리적으로 독립된 컴퓨터에서 
    # 실행하려면 main.py 수정, 동기화 및 추가 작업이 필요할 것입니다.
    #-------------------------------------------------------------------
    def _clients_training_with_redis(self, sampled_clients):
        ...
        for client_idx in sampled_clients:
            ...
            # 클라이언트에서 학습을 수행합니다.
            # 결과는 Redis 서버에 저장합니다.
            self.client.clients_training(client_idx)

            # Redis서버에 저장된 결과와 로컬 가중치를 읽어옵니다.
            local_results = rediscon.get_ordered_dict(f'fl:local_results:{client_idx}')
            local_size = rediscon.get_ordered_dict(f'fl:local_size:{client_idx}')
            local_weights = rediscon.get_ordered_dict(f'fl:local_weights:{client_idx}')
            
            # 서버에서 읽어온 로컬 가중치 값을 갱신합니다.
    ...
    def _set_client_data(self, client_idx):
        ...
        if is_redis:
            ...
            rediscon.set_data(f'fl:client:{client_idx}:datasize', self.client.datasize)
            #print( f'self.client.datasize = {self.client.datasize}' )
            
            self.client.train_idxs = self.data_distributed["local"][client_idx]["train_idxs"]
            rediscon.set_ordered_dict(f'fl:client:{client_idx}:train_idxs', self.client.train_idxs)
            #print( f'self.client.train_idxs = {self.client.train_idxs}' )
            
            self.client.test_idxs = self.data_distributed["local"][client_idx]["test_idxs"]
            rediscon.set_ordered_dict(f'fl:client:{client_idx}:test_idxs', self.client.test_idxs)
            #print( f'self.client.test_idxs = {self.client.test_idxs}' )
            
            self.client.data_name = self.data_distributed["data_name"]
            rediscon.set_data(f'fl:client:{client_idx}:data_name', self.client.data_name)
            #print( f'self.client.data_name = {self.client.data_name}' )
...
#         wandb.log(local_results, step=round_idx)#local model statistic
    ...
    def _update_and_evaluate(self, ag_weights, round_results, round_idx, start_time):
        ...
        if is_redis:
            # Debugging
            print(f'[d] type(ag_weights) = {type(ag_weights)}')
            print(f'[d] len(ag_weights) = {len(ag_weights)}')
                
            # Redis에 글로벌 모델 저장
            rediscon.set_ordered_dict(
                key = 'fl:server_weights',
                ordered_dict_model = ag_weights)
