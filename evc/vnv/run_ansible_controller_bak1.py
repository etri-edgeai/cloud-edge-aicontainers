...
def main(mode = 'baseline'):
    ...
    wdir = ' /home/jpark/www/cloud-edge-aicontainers/v3/vnv/app'
    py = ' /usr/bin/python3'
    ...
    print(f'[d] is_cuda_available = {is_cuda_available}')
    ...
    cmd = f'ansible server -i ./config/hosts.ini -m shell -a "cd {wdir}; pwd; {py} vnv.py --model {selected_model} --device {device} --N {N};"  {ask_pass_option} ' 
    run(cmd, True)
